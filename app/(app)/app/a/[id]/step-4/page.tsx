...
export default async function ApplicationStepFourPage({
  params,
  searchParams,
}: PageProps) {
  ...
  const schemaSubset = getSchemaSubsetForAppStep(schema, 4);

  return (
    <div className="rounded-2xl border border-zinc-200 bg-white p-6">
      <h1 className="text-xl font-semibold text-zinc-900">Passo 4</h1>
      <p className="mt-2 text-sm text-zinc-600">
        Trabalho/estudos e perguntas de segurança/antecedentes.
      </p>
      ...
      <AppStepFormClient
        ...
        initialValues={data ?? {}}
        nextPath={`/app/a/${resolvedParams.id}/review`}
        prevPath={`/app/a/${resolvedParams.id}/step-3`}
      />
    </div>
  );
}
